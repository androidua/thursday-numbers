...
def is_email_run(entry):
    ...
    Entries from v1.8.0 onward carry an explicit `source` field ("cron" |
    "local") — trust it. Legacy entries fall back to the weekday heuristic:
    the email cron fires Thursday 00:00 UTC and lands 01:00-03:00 UTC, so a
    Thursday generated_at means a delivered email for pre-v1.8.0 data.
    """
...
def build_scoreboard(picks_history, draws):
    ...
    for entry in picks_history:
        ...
        if draw is None:
            ...
            continue
        if draw["draw"] in seen_draw_nums:
            # Two email-classified entries matched the same draw (e.g. a legacy
            # local Thursday run) — scoring both would double-count the draw.
            skipped_duplicate.append(entry["generated_at"])
            continue
        seen_draw_nums.add(draw["draw"])
    ...
    return {
        ...
        "skipped_duplicate_draw": skipped_duplicate, # second entry for an already-scored draw
...
def print_summary(scoreboard):
    ...
    if scoreboard.get("skipped_duplicate_draw"):
        print(f"  Duplicates   : {len(scoreboard['skipped_duplicate_draw'])} entries "
              f"matched an already-scored draw and were excluded")
